...
def extract_letters(dilation, gray, thresh):
    ...
    # TODO: find third biggest to build prototype --> loop over all and throw away what is less than 70% and greater than 130% height
    prototype = sorted(key=lambda ctr: cv2.boundingRect(ctr)[3], reverse=True)[2]


    # import matplotlib.pyplot as plt; plt.imshow(roi); plt.show()
    return letter_rects
...
def recognize_plate(img, coords):
    ...
    if box_height <= box_width and box_height * 7 >= box_width:
        ...
        # extract letters from lp box
        letter_rects = extract_letters(dilation, gray, thresh)

        # TODO: apply cnn on letter_rects with some additional logic to get text
